...
class MarketDataTransformer:
    ...
    def calculate_rsi(self, period: int = 6):
    ...
    def calculate_sma(self, column: str, period: int):
        self.data[f'SMA_{period}_{column}'] = self.data[column].rolling(window=period).mean()
        return self
    ...
    def add_rsi_sma(self, period: int):
        self.calculate_sma('RSI', period)
        return self
    
    def add_price_sma(self, period: int):
        self.calculate_sma('close', period)
        return self
    
    def calculate_differences(self):
        self.data['Diff_EMA_RSI'] = self.data['EMA_9_RSI'] - self.data['EMA_21_RSI']
        self.data['Diff_SMA_Close'] = self.data['SMA_9_close'] - self.data['SMA_21_close']
        return self
    ...
    def transform(self):
        ...
        # Calcular SMA de 9 e 21 períodos para close
        self.add_price_sma(9)
        self.add_price_sma(21)
        
        # Calcular EMA de 9 e 21 períodos para RSI
        self.calculate_ema('RSI', 9)
        self.calculate_ema('RSI', 21)
        
        # Calcular SMA de 9 e 21 períodos para RSI
        self.add_rsi_sma(9)
        self.add_rsi_sma(21)
